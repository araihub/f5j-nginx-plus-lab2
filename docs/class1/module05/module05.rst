...
- ``リモートデスクトップを利用していない場合`` 
  ...
  - ご利用の端末から接続するため、 ``ubuntu01-nginx`` 、``ubuntu02-nginx`` の接続メニューより ``PLUS  DASHBOARD`` を選択してください

動作確認
----

``ubuntu01-nginx`` から、 ``ubuntu01-nginx(10.1.1.11)`` / ``ubuntu02-nginx(10.1.1.12)`` 双方に対して接続を行います。
以下の操作を行ってください
...
``ubuntu01-nginx`` 宛に接続した後、ステータス同期を待つため ``1秒停止(sleep 1)`` した後、``ubuntu02-nginx`` へ接続します
...
.. code-block:: bash
  ...
  $ echo "== To ubuntu01 =="
  == To ubuntu01 ==
  $ for i in {1..2}; do echo "==$i==" ; curl -I -s 10.1.1.11 ; done
  ==1==
  HTTP/1.1 200 OK
  Server: nginx/1.21.6
  ...
  $ echo "== To ubuntu02 =="
  == To ubuntu02 ==
  $ for i in {1..2}; do echo "==$i==" ; curl -I -s 10.1.1.12  ; done
  ==1==
  HTTP/1.1 503 Service Temporarily Unavailable
  Server: nginx/1.21.6
...
- ``ubuntu01-nginx`` に対して接続した結果を確認すると、5行目の1回目が ``200`` 、13行目の2回目が ``503`` となり RateLimitにより通信が拒否されています
- ``ubuntu02-nginx`` に対して接続した結果を確認すると、25行目の1回目、33行目が2回目の双方が ``503`` となり RateLimitにより通信が拒否されています。これはステータスが同期されたためこのような動作となります
...
- ``HTTP Zones`` のタブの内容を開きます。左が ``ubuntu01-nginx`` 、右が ``ubuntu02-nginx`` となります。
- 通信の結果、 ``5xx`` の結果を確認すると ``ubuntu01-nginx`` に ``1`` 、 ``ubuntu02-nginx`` が ``2`` となっていることがわかります
...
``ubuntu01-nginx`` 、 ``ubuntu02-nginx`` で以下の操作を行ってください
...
.. code-block:: bash
  ...
  keyval_zone zone=iplist:32k state=/var/lib/nginx/state/appservers.conf timeout=300s sync;
...
``ubuntu01-nginx`` で ``iplist`` という名称のKeyValが生成されていることが確認できます
...
``ubuntu01-nginx`` でサンプルのリクエストを送信し、KeyVal の結果を確認します
...
``ubuntu01-nginx`` でURLパラメータに ``user`` を含むサンプルのリクエストを送信し、KeyVal の結果を確認します
...
- ``iplist`` に ``ubuntu01-nginx`` のIPアドレス 10.1.1.11 が追加されていることが確認できます

``ubuntu02-nginx`` でURLパラメータに ``user`` を含むサンプルのリクエストを送信し、KeyVal の結果を確認します
...
- ``iplist`` をみると ``ubuntu01-nginx`` のIPアドレス 10.1.1.11 が追加されており、 ``ubuntu02-nginx`` のアドレスと一致しないためエラーとなったことがわかります
...
一定時間経過後(今回の設定サンプルでは300秒)で各エントリが消えますが、そのタイミングで値は変化しません。
